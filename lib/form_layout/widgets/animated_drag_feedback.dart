...
class _AnimatedDropTargetState extends State<AnimatedDropTarget>
  ...
  void initState() {
    ...
    _scaleAnimation = Tween<double>(begin: 1.0, end: widget.activeScale)
        .animate(
          CurvedAnimation(
            parent: _controller,
            curve: widget.animationSettings.defaultCurve,
  ...
  void didUpdateWidget(AnimatedDropTarget oldWidget) {
    ...
    if (widget.isActive != oldWidget.isActive &&
        widget.animationSettings.enabled) {
      if (widget.isActive) {
        _controller.forward();
      } else {
        _controller.reverse();
  ...
  Widget build(BuildContext context) {
    if (!widget.animationSettings.enabled) {
      return widget.isActive
          ? Transform.scale(scale: widget.activeScale, child: widget.child)
          : widget.child;
    }
